        ...
        for (Bundle.BundleEntryComponent entry : fhirDocument.getEntry()) {
            ...
            if (entry.getResource() instanceof Encounter) {
                ...
                for (Encounter.EncounterParticipantComponent participantComponent : encounter.getParticipant()) {
                    if (participantComponent.hasIndividual())
                        participantComponent.setIndividual(getUUIDReference(participantComponent.getIndividual()));
                }
                if (encounter.hasPartOf()) {
                    encounter.setPartOf(getUUIDReference(encounter.getPartOf()));
                }
            }
            ...
            if (entry.getResource() instanceof MedicationRequest) {
                ...
                if (medicationRequest.hasRecorder())
                    medicationRequest.setRecorder(getUUIDReference(medicationRequest.getRecorder()));
                try {
